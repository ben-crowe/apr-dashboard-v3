...
1. **Valcre strategy = MODIFY the one test job for field mapping; CREATE freely (batched) when needed (revised w/ Ben 2026-06-03).** Most field-mapping tests need NO new jobs — use the existing test job **VAL261101 "Westside Mall"** and override it ONE FIELD AT A TIME to verify each mapping. When we DO need to create jobs (e.g. testing the create path, new custom fields), that's fine — Chris is OK with test jobs and Ben deletes them after; just do ALL Valcre creation in ONE dedicated batch session, not dribbled out. Don't contort the integration to avoid making jobs.
2. **For NOW, the only Valcre test = make ONE job, confirm it returns a proper Valcre number + basic map. Done.** Full field mapping / new custom fields / add-after-create sync = the dedicated Valcre batch session later (done largely by modifying VAL261101 field-by-field). Light safety note (not a heavy gate): VAL numbers can get reassigned to real clients (VAL261028→Bentley Multifamily, VAL251031→Barrie Townhomes), so before modifying, sanity-check the job still reads "Westside Mall." Valcre auth WORKS (HTTP 200, valid token) — the exit-3 was a `$filter` URL-encoding bug in `valcre-verify-job.sh`, NOT an auth problem (root cause: `tests/ROOTCAUSE-valcre-auth-and-pinjob-2026-06-03.md`). Fix = the script's `$filter` encoding.
...
5. **ClickUp card = built in 3 STAGES; CURRENT card ≠ the rich target (re-corrected 2026-06-03 via qa code+live analysis — `tests/MAPPING-dashboard-to-clickup.md`).** The CURRENT deployed builders produce a card that is **lean in Stage-1 + rich in Stage-2 + a status tracker**:
   - **Stage-1 (intake card) — LEAN, 5 lines only:** Client / Property / Type / Intended Use / Notes. Built by TWO parallel builders (`supabase/functions/create-clickup-task` AND client `src/utils/webhooks/clickup.ts`) — duplicate, dedupe candidate (clickup.ts is the CORS-failing client copy).
   - **Stage-2 — RICH:** `supabase/functions/update-clickup-task` appends LOE QUOTE & VALUATION (property_rights_appraised, scope_of_work, report_type, appraisal_fee, retainer_amount, delivery_date, payment_terms) + APPRAISER NOTES (internal/delivery/payment/client comments) + the Valcre link.
   - **Status tracker — RICH:** `supabase/functions/docuseal-webhook` fills Received / LOE Sent / LOE Signed.
   - **`create-valcre-job` does NOT touch the ClickUp card** (its rich Property Contact / Organization content goes to VALCRE, not the card).
   - **MAJOR GAP:** the rich Stage-1 sections in the canonical target (the Summit Tower example `~/Development/KM-Exp/data/tasks/content/task_4j4l1r3v/attachments/clickup-task-example.png` — full CLIENT INFO / PROPERTY INFO / PROPERTY CONTACT / CLIENT COMMENTS) are produced by NO current builder. The Summit Tower card is the TARGET, not current output. **Build task: enrich the Stage-1 card to match.**
   - **Live render UNVERIFIED:** VAL261101's card (task 86b8c5xt9) 404s with the BC test token (it's a prod-workspace task) — Stage-2 VALUES confirmed at Valcre source, but the rendered card couldn't be checked until auto-ClickUp points at the test list. Prod list `901402094744` = Chris's, do NOT write.
...
8. **Valcre has NO test env** — every call is prod-as-Chris. `valcre-auth.sh` WORKS (HTTP 200, valid token). The earlier exit-3 was a `$filter` URL-encoding bug in `valcre-verify-job.sh` (qa root-caused) — fix that script's encoding; auth itself is fine. Missing scripts: `valcre-get-by-number`, `valcre-delete-job` (prod-risk; rely on modify-not-create), `valcre-reset-test-job`.

9. **⭐ CODEX IS A FIRST-CLASS TEAMMATE + OWNER OF THE LOE/REPORT HTML TEMPLATES — KNOW THIS ON EVERY READ (locked w/ Ben 2026-06-10).** Codex (GPT-5.5) runs as a real CLI teammate in its own tmux pane (`sh-1`) with its own checkpoints, AND as desktop Computer-Use with its own internal browser. **It authored the LOE engagement-letter HTML and is the registered OWNER of the LOE/report templates.** Ben's standing direction: it is the **go-to agent for the hard HTML / report-template / visual / spatial work** — its vision + spatial capability is night-and-day ahead (it nearly one-shotted the whole LOE contract HTML with almost no fixes, and its browser render matched qa's exact LOE-PDF readout).
   - **TESTING RULE (so neither qa nor Ben forgets the capability):** when ANY finding lands in the LOE/report **HTML-template layer** (blank rows, token/merge issues, spacing, layout, visual fidelity), the report must say **"this is a Codex fix — route to Codex,"** not just describe the bug. qa surfaces "we have Codex for this"; Ben confirms Codex is ready and hands it the prompt.
   - **DIVERGENCE GUARD (load-bearing):** the LIVE template is the **Supabase `loe_templates` DB row**, NOT the static `LOE-template-v07.html`. Any direct DB/file patch puts Codex's copy behind. So BEFORE handing Codex a template change, **export the current live template to a file** (qa can pull it from `loe_templates`) and give Codex THAT — it edits from current truth, then pushes back to the DB row (saving the file alone is not a deploy). Canonical live-export landing: `docs/Features/12-LOE-Esign/loe07-build/LIVE-loe-template-v07-current.html`.
   - **VIEW CODEX'S VISUAL OUTPUT:** desktop Codex shows its render in its own browser (great for spacing). For CLI Codex, surface its output as a **PDF in the KM viewer** (Ben has PDF viewing) — just as good for reviewing visual/spatial results.
   - Codex pane comms: `tmux send-keys -t sh-1 "<single-line msg>" Enter` (its CLI submits on newline — keep messages one line). Computer-Use operator role (Phase 1/2 form-fill + Phase 6 DocuSeal sign) per item 7. Capability notes: `~/.claude/learnings/codex.txt`.

10. **⭐ LOE / REPORT RENDER-REVIEW GATE — run this on EVERY LOE grade, BEFORE sign-off (added 2026-06-10 after qa MISSED the Schedule-A leak).** Verifying one assigned section is NOT a render review. A real LOE review audits the WHOLE letter for conditional-section + placeholder leakage. The miss: qa verified §10 EAHC suppression hard but never generalized the check — Schedule A (same conditional-section class) was rendering on a single-property letter and shipped through the grade. ROOT CAUSE pattern (both §10 and Schedule A): the app's `applyConditionalSection`/`applyConditionalScheduleA` strip a block by grepping `<!-- FENCE:START -->...<!-- FENCE:END -->` markers — and the LIVE Supabase `loe_templates` DB template is MISSING those markers (the static `LOE-template-v07.html` HAS them; the DB copy was never synced). Missing markers → nothing strips → the block + its placeholder text render always.
   - **MANDATORY checks every LOE grade:** (a) **Schedule A** present ONLY when AssignmentType contains "Multiple" — on a single-property job it must be GONE (not showing "THIS SCHEDULE INSERTED WHEN … SELECTED TO: MULTIPLE PROPERTIES"). (b) **§10 EAHC** rows = exactly the derived scenario count, 0 blank rows; whole §10 dropped if 0 scenarios. (c) **Scan the FULL rendered PDF for ANY leaked placeholder** — `grep` the rendered text for `[`...`]` tokens (e.g. `[EA/HCSummary`), the literal strings `INSERTED WHEN` / `THIS SCHEDULE`, and any `merge-token`/`data-token` that didn't resolve. ANY hit = FAIL. (d) Every conditional section the code can suppress (Schedule A, §10, future ones) must have its `<!-- FENCE:START/END -->` markers present in the LIVE DB template — if a suppress rule exists in code but the markers are absent in the DB row, that's the bug. Route the fix to **Codex** (template-layer owner) — add the missing fence markers to the Supabase `loe_templates` row, NOT the static file.
   - **Render gotcha (banked):** the LOE template is print/Paged.js media — `file://` loads silently fail in playwright (lands on `about:blank` → blank capture). Serve over `http` + use playwright `pdf` (not screen screenshot) to get the real letter; `pdftotext`/`pdftoppm` to audit.
...
- **The task is a rich markdown CARD built by EDGE FUNCTIONS (not the dead clickup.ts), not custom fields:** generated/evolved by `supabase/functions/update-clickup-task` + `docuseal-webhook`, using the production APR Dashboard URL. No custom-field-count work. `src/utils/webhooks/clickup.ts` is legacy to remove.

**Follow-ups logged (not blocking the first run):**
- Clean up the DEAD `src/utils/webhooks/clickup.ts` (legacy lean client-side path, superseded by the Supabase edge functions). Confirm nothing live still imports it before removing.
- Canonical ClickUp card template = the filled Summit Tower example (`~/Development/KM-Exp/data/tasks/content/task_4j4l1r3v/attachments/clickup-task-example.png`). The card is the markdown description from the edge functions — no field-count or 49-field reconciliation work needed.
...
| 2 — ClickUp task created | Screenshot of created task in BC test workspace | Task ID + URL stored in DB; name matches `VAL<num> - <property>`; the **Stage-1 intake card** renders (header links + Client/Property/Property Contact/Comments) per the canonical Summit Tower example; APR Dashboard link resolves. Stages 4/7 then verify the LOE Quote/Valuation + Valcre link + LOE Signed sections fill (edge-function output). |
