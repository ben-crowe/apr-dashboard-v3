...
5. **The "49 custom fields" figure is LEGACY — NOT the current model (corrected w/ Ben 2026-06-03).** Pre-dashboard, ALL client-submission fields were dumped into ClickUp custom fields because there was nowhere else to put them. Now most data lives in the APR DASHBOARD; the ClickUp task is meant to be MINIMAL — a preview + links back to the dashboard/Valcre. So Phase-2 must assert the CURRENT intended minimal field set (TBD — define it), NOT 49. The app's create-task mapping (still built from the ~49 prod set) is likely legacy and needs trimming. Prod list `901402094744` = Chris's, do NOT write.
...
- **Field set is MINIMAL-by-design now (NOT 49):** the 49-field model is legacy (pre-dashboard dump). Today the ClickUp task should carry only a minimal preview set + links to the APR dashboard/Valcre — most data lives in the dashboard. Real task: define the CURRENT correct minimal field set and reconcile the create-task mapping + Phase-2 assertion to it. (Follow-up.)

**Follow-ups logged (not blocking the first run):**
- Define the CURRENT minimal ClickUp field set (post-dashboard model = preview + links to dashboard/Valcre, NOT the legacy 49-field dump). Reconcile the app's create-task mapping + the Phase-2 assertion to it, and confirm what the test list's 27 fields should actually be (some may be legacy leftovers).
...
| 2 — ClickUp task created | Screenshot of created task in BC test workspace | Task ID + URL stored in DB, name matches `VAL<num> - <property>`, 49 custom fields populated |
