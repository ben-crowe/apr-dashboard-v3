...
- `[~]` **2b. Asset Storage** — Supabase image buckets done; SharePoint per-job folder-create `[ ]` NOT wired. At intake the app creates a parent job folder under `…/sites/V/Shared Documents/2.Jobs/{YEAR}/` **AND scaffolds 5 standard subfolders inside it**. Site: `valtapropertyvaluations.sharepoint.com/sites/V`.
