...
6. **Valcre — map 1:1, DELETE the parser (this is the simplification + it FIXES the truncated-street bug):**
   - **UPDATE path `api/valcre.ts`** (~L1010): set `AddressStreet=clientAddress`, `AddressCity=clientCity`, `AddressState=clientProvince`, `AddressPostalCode=clientPostal` **directly** — REMOVE the `parseAddress(clientAddress)` call for the contact (it took only the first comma-segment as street → dropped "1776 Maple Way"; gone now). Same 1:1 for the property entity (~L1179).
   - **⚑ PropertyContact (~L1099) — THIRD address sink, PIN IT DELIBERATELY (QA re-gate 2026-06-16):** PropertyContact is a separate PERSON/Contact entity (`PropertyContactId`). Today `AddressStreet` falls back to `addressParts.street` (L1100) — i.e. the PARSED client address — which is exactly the cross-contamination the no-cross rule forbids, and a parseAddress-deletion fall-through would silently keep it. **DECISION — ✅ BEN-CONFIRMED 2026-06-16 (no override): PropertyContact gets NO address — leave all four address fields EMPTY.** Rationale: we do not collect a property-contact address in the data model, so populating it from the client's OR property's parts is invented cross-data. Remove the `|| addressParts.street` fallback; do NOT substitute client or property parts. (If Ben later wants the property contact to carry the PROPERTY's address, that's a deliberate explicit map — not a fall-through.)
...
4. **Valcre 1:1 — readback-verify ALL THREE entities (Contact, Property, AND PropertyContact)** in Valcre: `AddressStreet`/`AddressCity`/`AddressState`/`AddressPostalCode` each hold the matching field exactly, no parsing artifacts, street NOT truncated. **EXPLICIT no-cross check (use DIFFERENT client vs property addresses on the test job so any swap is visible):** the Contact holds the CLIENT's address; the Property holds the PROPERTY's address; **PropertyContact holds NO address (empty)** — confirm none are swapped and PropertyContact did NOT inherit the client's or property's address via fall-through.
