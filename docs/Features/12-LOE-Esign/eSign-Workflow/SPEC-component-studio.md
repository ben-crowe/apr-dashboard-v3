...
**INV-4 — The split is render-LEFT / editor-RIGHT, render-dominant by default, with expand + drag + zoom; the editor is INLINE WYSIWYG (Ben chose option b, 2026-06-18).**
  ...
  - **Inline-editor scope (Ben, 2026-06-18 — revised: ALL THREE inline for consistency):** **Document, Email AND Popup all edit INLINE in the right pane** — the SAME `designMode` editable surface, never a raw-HTML box. Email/Popup Save writes the template in place via `saveEmailTemplate` / `savePopupTemplate`; **Document Save persists a tailored draft via the EXISTING `saveJobContract` path** (no new save engine), with the left render still through the shared `DocumentPreviewPane` + `generateLOEHTML` — so **INV-0 stays intact** (no document render/save rebuild). A small **"Open full editor ↗"** link remains on the Document for heavy/structural edits (the full `TemplateEditorModal`).
  - PROVED-BY (inline): open Email/Popup/Document → the right pane is the editable rendered surface (token chips for email/popup); edit + Save persists via the existing path for that type (`saveEmailTemplate` / `savePopupTemplate` / `saveJobContract`) — grep confirms no new save engine; the Document "Open full editor ↗" still launches the existing `TemplateEditorModal`.
