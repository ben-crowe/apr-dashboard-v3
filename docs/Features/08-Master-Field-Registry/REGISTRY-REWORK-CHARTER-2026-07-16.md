...
**SCOPE OF THE MATCHING RULES (Ben):** the no-superset / same-id rules bind ONLY our family — V3, V4, and the client's registry once it adopts our ids. VALCRE IS EXEMPT: longer value lists, different ids, different field names are all EXPECTED there and absorbed by the one border mapping file — never flag Valcre differences as drift. And where the client deliberately chose an improved field name that differs (from Valcre or from history), that is a DECISION, not a mismatch — do not "fix" it to match.
