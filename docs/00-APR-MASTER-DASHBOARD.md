...
| — | **APR V4 (Report Builder)** | Stage-4 report builder — now an **ACTIVE workstream** (no longer parked). Deployed to its OWN site (`apr-v4.vercel.app`, flag-gated `VITE_V4_ENABLED`); the canonical V3→V4 flow + completion plan are written; V3-origin tab field alignment shipped 2026-06-21. The 5-stage spine: V3 done-job → "Create Report" button (to build) → `useLoadJobIntoReport` map → V4 builder (separate field area) → 79-pg template. | [**★ V4 Completion Plan**](~/Development/APR-Dashboard-v3/docs/Architecture/V4-COMPLETION-PLAN.md) · [V4 Image-Library Plan](~/Development/APR-Dashboard-v3/docs/Architecture/V4-IMAGE-LIBRARY-PLAN.md) · [V3↔V4 Field Delta](~/Development/APR-Dashboard-v3/docs/Architecture/V3-V4-FIELD-ALIGNMENT-DELTA.md) · `docs/Features/07-Report-Builder/` |
...
- `[~]` **2b. Asset Storage + Email (one Microsoft 365 / Graph integration)** — ONE Entra app powers folders AND email send. Email-send detail is the "Email deliverable" line in the section below (same app: add `Mail.Send` alongside `Sites.ReadWrite.All`). SharePoint side: Supabase image buckets done; per-job folder-create `[x]` WIRED + VERIFIED (2026-06-23) — the **"Asset Folders" button** in the job's LOE section creates the full 5-subfolder set live via the SharePoint App (visually confirmed on VAL261066: 1-REPORT, 2-CLIENT SUPPLIED, 3-WORK FILES, 4-CLIENT BILLING (Invoice, LOE), 5-LOR). OPEN: confirm whether create also auto-fires at intake (vs button-only), and wire signed-LOE auto-drop into folder 4. At intake the app creates a parent job folder under `…/sites/V/Shared Documents/2.Jobs/{YEAR}/` **AND scaffolds 5 standard subfolders inside it**. Site: `valtapropertyvaluations.sharepoint.com/sites/V`.
...
- `[ ]` **★ NEXT PRIORITY — Login + Roles (low-friction auth)** — Ben (2026-06-22): the client + his team start using the app within days and don't want it to feel like a public, anyone-can-open URL; a login makes their data feel private, AND role-based access retires the our-version-vs-their-version URL/flag hack (one site, gated by who you are). Low-friction is the hard requirement (persistent sessions / magic-link / OAuth — no per-visit annoyance). Intake form stays public; dashboard + report builder get gated. **STATUS: DRAFT PRD written — NOT scheduled. Next step = qa-agent reviews the PRD + the core-agent choice + the deploy prompt BEFORE any build; Ben sets go-time.** See [PRD — Login + Roles](~/Development/APR-Dashboard-v3/docs/Architecture/PRD-AUTH-ROLES.md).
- `[~]` **— APR V4 (Report Builder)** — NO LONGER PARKED (active as of 2026-06-21). Deployed standalone (`apr-v4.vercel.app`); field alignment to V3 shipped + committed; deploy data-layer fixed (Supabase URL/key trailing-newline bug). NEXT (completion-plan order): fill V4 w/ same test data as V3 = playable standalone (recon $0 value-flow + missing-field/LOE-cascade gaps) → then wire the "Create Report" V3→V4 handoff. See the [V4 Completion Plan](~/Development/APR-Dashboard-v3/docs/Architecture/V4-COMPLETION-PLAN.md).
