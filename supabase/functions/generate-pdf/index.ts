...
serve(async (req) => {
  ...
  try {
    ...
    const GOTENBERG_URL = Deno.env.get('GOTENBERG_URL') || 'http://localhost:3000'
    ...
    if (error.message?.includes('connection') || error.message?.includes('ECONNREFUSED')) {
      return new Response(
        JSON.stringify({
          ...
          details: 'Gotenberg service is not running. Start with: docker run -p 3000:3000 gotenberg/gotenberg:8',
