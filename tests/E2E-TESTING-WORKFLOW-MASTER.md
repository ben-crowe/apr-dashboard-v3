...
5. **ClickUp task = a RICH, MULTI-STAGE markdown card built by SUPABASE EDGE FUNCTIONS (corrected 2026-06-03 w/ Ben — earlier "lean card / zero fields" note was from a DEAD file).** Canonical output = the filled example `~/Development/KM-Exp/data/tasks/content/task_4j4l1r3v/attachments/clickup-task-example.png` (VAL251032 Summit Tower): header links (**APR Dashboard** + **VALCRE Job Number**) · status tracker (**Received / LOE Sent / LOE Signed**) · **CLIENT INFO** · **PROPERTY INFO** · **PROPERTY CONTACT** · **CLIENT COMMENTS** · **LOE QUOTE & VALUATION** (Property Valuation + Financial Details) · **APPRAISER NOTES**. Built in STAGES that EVOLVE the same task:
   - Stage-1 intake card (client/property/contact/comments) — created on submission.
   - `supabase/functions/update-clickup-task/index.ts` — appends LOE Quote & Valuation / Financial / Appraiser Notes + the **Valcre link** when the job is booked.
   - `supabase/functions/docuseal-webhook/index.ts` — fills **Received / LOE Sent / LOE Signed** as e-sign progresses.
   - Links live IN `markdown_description` (the fn: "use ONLY markdown_description for clickable links"). It uses the production APR Dashboard URL.
   **`src/utils/webhooks/clickup.ts` is DEAD legacy** (lean client-side path, CORS-fails) — do NOT reference it; clean it up. The 27/49 custom-field counts are irrelevant — the card is the markdown description, not custom fields. Prod list `901402094744` = Chris's, do NOT write.
